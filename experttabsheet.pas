...
begin
  try
    ...
    if GetParameter(Parameters,'CloseBtn') <> EmptyStr then
      CloseBtn := StrToBool(GetParameter(Parameters,'CloseBtn'))
    else
      CloseBtn := True;
...
begin
  try
    ...
    if FileExists('CloseBtn.bmp') then
      begin
        FCloseBtn.Glyph.LoadFromFile('CloseBtn.bmp');
        FCloseBtn.Flat := True;
      end
    else
      FCloseBtn.Caption := 'X';
