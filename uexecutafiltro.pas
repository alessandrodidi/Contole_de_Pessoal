...
var
  frmExecutaFiltro: TfrmExecutaFiltro;

implementation

Uses
  UGFunc, UFiltros, UDBO, UConexao, URegLogsSistema, UCadPessoas;

{$R *.lfm}

{ TfrmExecutaFiltro }

procedure TfrmExecutaFiltro.FormShow(Sender: TObject);
var
  i,n,topPos, topPos2: Integer;
  Valor, Valor_Final, Filtro: String;
  CamposDinamicos: Boolean;
begin
  try
    //Se não tiver filtro selecionado abre o gerenciador de filtros
    ...
    //Caso haja filtro selecionado executa o filtro
    Filtro := EmptyStr;
    SQLQuery(sqlqExpressoes,['SELECT *'
                            ,'FROM g_filtros_expressoes E'
                            ,'INNER JOIN g_filtros F'
                                   ,'ON F.id_filtro = E.id_filtro'
                            ,'WHERE E.id_filtro = '''+ID_Filtro+''''
                            ,'ORDER BY E.ordem, E.id_expressao']);

    Modulo := sqlqExpressoes.FieldByName('modulo').Text;
    Formulario := sqlqExpressoes.FieldByName('formulario').Text;
    ...
    while not sqlqExpressoes.EOF do
      begin
        ...
        //Verifica se em alguma das linhas da expressão existem campos dinâmicos a serem montados
    ...
    //Se hoverem campos dinâmicos executa a rotina de montagem dos campos
...
procedure TfrmExecutaFiltro.btnExecutarClick(Sender: TObject);
var
  i,j: Integer;
  Expressao, CriterioF, CondicaoF, CampoF, Valor1, Valor2, ValorR1, ValorR2, ValorF: String;
  ...
  try
    {if CamposDinamicos then
        begin
          ...
          SetLength(frmExecutaFiltro.MontaFiltro,High(MontaFiltro)+1,9);

          for j:=0 to High(MontaFiltro) do
            begin
              frmExecutaFiltro.MontaFiltro[j,0] := MontaFiltro[j,0];
              frmExecutaFiltro.MontaFiltro[j,1] := MontaFiltro[j,1];
              frmExecutaFiltro.MontaFiltro[j,2] := MontaFiltro[j,2];
              frmExecutaFiltro.MontaFiltro[j,3] := MontaFiltro[j,3];
              frmExecutaFiltro.MontaFiltro[j,4] := MontaFiltro[j,4];
              frmExecutaFiltro.MontaFiltro[j,5] := MontaFiltro[j,5];
              frmExecutaFiltro.MontaFiltro[j,6] := MontaFiltro[j,6];
              frmExecutaFiltro.MontaFiltro[j,7] := MontaFiltro[j,7];
              frmExecutaFiltro.MontaFiltro[j,8] := MontaFiltro[j,8];
           end;
    ...
    //Rotina para o formulário de Logs do Sistema
    if ((Modulo = 'AUDITORIA')
       and (Formulario = 'LOGSSISTEMA')) then
      begin
        frmRegLogsSistema.ID_Filtro_Selecionado := ID_Filtro;
        frmRegLogsSistema.Filtro := Expressao;
        frmRegLogsSistema.sbtnAtualizar.Click;
      end;

    //Rotina para o formulário de Cadastro de Pessoas
    if ((Modulo = 'CADASTROS')
       and (Formulario = 'CADPESSOAS')) then
      begin
        frmCadPessoas.ID_Filtro_Selecionado := ID_Filtro;
        frmCadPessoas.Filtro := Expressao;
        frmCadPessoas.sbtnAtualizar.Click;
      end;
...
procedure TfrmExecutaFiltro.btnCancelarClick(Sender: TObject);
begin
  Self.Close;
end;

procedure TfrmExecutaFiltro.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_RETURN then
    if btnExecutar.Focused then
      btnExecutar.Click
    else
      Key := VK_TAB;
